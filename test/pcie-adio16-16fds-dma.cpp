...
void * worker_main(void *arg)
{
	...
	do
	{
		...
		status = AIOWDM::DmaDataReady(0, &first_slot, &num_slots, &data_discarded);
		...
		for (int i = 0 ; i < num_slots ; i++)
		{
			...
			memcpy(ring_buffer[(first_slot + i) % RING_BUFFER_SLOTS],
							(const void *)DmaBase + (BYTES_PER_TRANSFER * ((first_slot + i) % RING_BUFFER_SLOTS)),
							BYTES_PER_TRANSFER);
			pthread_mutex_unlock(&ring_logger_mutex);
			sem_post(&(ring_sem));
...
void set_acquisition_rate (int fd, double *Hz)
{
	...
	base_clock = AIOWDM::RelInPortL(0, BASECLOCKOFFSET);
...
int main (void)
{
	...
	if (status)  {
		printf("Error setting transfer_size: status = 0x%d, errno = %d\n", status, errno);
		return -1;
	}
	...
	AIOWDM::RelOutPortL(0, FAFIRQTHRESHOLDOFFSET, FIFO_SIZE);
	//apci_read32(fd, 1, BAR_REGISTER, FAFIRQTHRESHOLDOFFSET, &depth_readback);
	depth_readback = AIOWDM::RelInPortL(0, FAFIRQTHRESHOLDOFFSET);
	...
	AIOWDM::RelOutPortL(0, ADCRANGEOFFSET, 0);

	//enable ADC Trigger IRQ
	//apci_write32(fd, 1, BAR_REGISTER, IRQENABLEOFFSET, bmADIO_ADCTRIGGEREnable|bmADIO_DMADoneEnable);
	AIOWDM::RelOutPortL(0, IRQENABLEOFFSET, bmADIO_ADCTRIGGEREnable|bmADIO_DMADoneEnable);
	...
	AIOWDM::RelOutPortL(0, ADCCONTROLOFFSET + 4, start_command);
	//apci_write32(fd, 1, BAR_REGISTER, ADCCONTROLOFFSET, start_command);
	AIOWDM::RelOutPortL(0, ADCCONTROLOFFSET, start_command);
