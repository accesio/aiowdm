...
struct accesio_pci_device_context
{
    ...
    int                                  waiting_for_irq;
    int                                  irq_cancelled;
    long                                 irq_return; //is returned by WaitForIRQ
};
...
long ioctl_ACCESIO_PCI_DMA_INIT (struct accesio_pci_device_context *context, unsigned long arg)
{
    ...
    if (dma_context->dma_virt_addr != NULL) {
        dma_free_coherent(&context->pci_dev->dev,
                        ...
                        dma_context->dma_virt_addr,
                        dma_context->dma_addr);

        dma_context->dma_num_slots = 0;
        dma_context->dma_virt_addr = NULL;
        dma_context->dma_addr = 0;
    ...
    if (dma_context->dma_virt_addr == NULL) status = -ENOMEM;

    dma_context->dma_last_buffer = -1;
...
long ioctl_ACCESIO_PCI_DMA_DATA_TAKEN (struct accesio_pci_device_context *context, unsigned long arg)
{
    ...
    spin_lock_irqsave(&dma_context->dma_data_lock, flags);
    dma_context->dma_first_valid += arg;
    dma_context->dma_first_valid %= dma_context->dma_num_slots;
    spin_unlock_irqrestore(&dma_context->dma_data_lock, flags);
    return 0;
}
...
//What mmap tries to accomplish depends on the value sent for offset when calling
//the mmap() from userspace. It is received as vma->vm_pgoff
int accesio_char_driver_mmap (struct file *filp, struct vm_area_struct *vma)
{
    struct accesio_pci_device_context *ddata = filp->private_data;
    int status = 0;
    aio_driver_debug_print("<<<");
    switch (vma->vm_pgoff)
    {
